...
const activeTab = ref('files') // 'files' or 'windows'

const fileSessions = computed(() => sessions.value.filter(s => s.has_file))
const windowsSessions = computed(() => sessions.value.filter(s => s.has_session))
...
async function massKill() {
  const targetSessions = activeTab.value === 'files' ? fileSessions.value : windowsSessions.value;
  if (targetSessions.length === 0) return
  
  const typeText = activeTab.value === 'files' ? 'archivos y sesiones de Novasoft' : 'sesiones activas de Windows';
  
  openConfirm(
    'Cerrar TODAS las sesiones',
    `¿Estás absolutamente seguro de cerrar las <strong>${targetSessions.length}</strong> ${typeText}? Esta acción desconectará a los usuarios de forma inmediata.`,
    'Cerrar Todo',
    'bg-red-600 hover:bg-red-700 text-white',
    async () => {
    ...
    <div class="grid grid-cols-1 lg:grid-cols-4 gap-4 flex-1 overflow-hidden">
      ...
      <div class="lg:col-span-3 bg-white border border-slate-200 rounded-sm shadow-sm overflow-hidden flex flex-col">
        <div class="p-3 border-b border-slate-200 bg-slate-50 flex items-center justify-between">
          ...
          <button @click="massKill" :disabled="(activeTab === 'files' ? fileSessions.length : windowsSessions.length) === 0" class="border border-red-600 text-red-600 hover:bg-red-50 px-3 py-1 rounded-sm text-[11px] font-bold transition-colors disabled:opacity-50">
            CERRAR TODO EN PESTAÑA ACTUAL
          </button>
        </div>
        ...
        <div class="flex border-b border-slate-200 bg-white">
          <button 
            ...
            :class="[
              'flex-1 py-3 px-4 text-sm font-semibold text-center border-b-2 transition-colors',
              activeTab === 'files' ? 'border-indigo-500 text-indigo-600 bg-indigo-50/50' : 'border-transparent text-slate-500 hover:text-slate-700 hover:bg-slate-50'
            ]"
          >
            Archivos de Control (Novasoft)
            <span class="ml-1.5 inline-flex items-center justify-center bg-slate-100 text-slate-600 px-2 py-0.5 rounded-full text-xs border border-slate-200">
              {{ fileSessions.length }}
            </span>
          </button>
          <button 
            ...
            :class="[
              'flex-1 py-3 px-4 text-sm font-semibold text-center border-b-2 transition-colors',
              activeTab === 'windows' ? 'border-indigo-500 text-indigo-600 bg-indigo-50/50' : 'border-transparent text-slate-500 hover:text-slate-700 hover:bg-slate-50'
            ]"
          >
            Sesiones de Windows (RDS)
            <span class="ml-1.5 inline-flex items-center justify-center bg-slate-100 text-slate-600 px-2 py-0.5 rounded-full text-xs border border-slate-200">
              {{ windowsSessions.length }}
            </span>
          </button>
        </div>
        
        <!-- Files Tab Content -->
        <div v-if="activeTab === 'files'" class="flex-1 overflow-auto">
          <table class="w-full text-left text-[13px]">
            <thead class="bg-slate-50 border-b border-slate-200 sticky top-0 z-10 text-slate-600 font-semibold uppercase tracking-wider text-[11px]">
              <tr>
                <th class="px-5 py-3">Usuario (Archivo)</th>
                <th class="px-5 py-3">Usuario (AD)</th>
                <th class="px-5 py-3">Sesión Vinculada</th>
            ...
            <tbody class="divide-y divide-slate-100 text-slate-700">
              <tr v-if="loading" class="bg-white">
                <td colspan="4" class="px-5 py-12 text-center">
                  ...
                  <p class="text-sm text-slate-500 font-medium">Buscando archivos temporales...</p>
                </td>
              </tr>
              <tr v-else-if="fileSessions.length === 0" class="bg-white">
                <td colspan="4" class="px-5 py-12 text-center text-slate-500">
                  ...
                  <p class="text-[13px] font-medium">No hay archivos atrapados.</p>
                  <p class="text-[12px] text-slate-400 mt-1">El sistema Novasoft está limpio.</p>
                </td>
              </tr>
              <tr v-for="session in fileSessions" :key="session.username" class="hover:bg-slate-50 transition-colors">
                <td class="px-5 py-2">
                  <div class="font-medium text-slate-800">{{ session.username }}</div>
                  <div class="text-[10px] text-slate-400 font-mono mt-0.5">Archivo: {{ schedule.file_prefix }}{{ session.username }}</div>
                </td>
                <td class="px-5 py-2 font-mono text-slate-600">{{ session.full_name }}</td>
                <td class="px-5 py-2">
                  <span v-if="session.has_session" class="inline-flex items-center gap-1.5 px-2 py-0.5 rounded text-[10px] font-medium border bg-emerald-50 text-emerald-700 border-emerald-200">
                    <span class="w-1.5 h-1.5 rounded-full bg-emerald-500"></span>
                    ID: {{ session.session_id }} ({{ session.state }})
                  </span>
                  <span v-else class="inline-flex items-center gap-1.5 px-2 py-0.5 rounded text-[10px] font-medium border bg-amber-50 text-amber-700 border-amber-200">
                ...
                <td class="px-5 py-2 text-right">
                  <button 
                    ...
                    class="border border-red-600 text-red-700 hover:bg-red-50 px-3 py-1 text-[11px] rounded-sm font-medium transition-colors disabled:opacity-50 inline-flex items-center gap-1 ml-auto min-w-[120px] justify-center"
                  >
        ...
        <!-- Windows Tab Content -->
        <div v-if="activeTab === 'windows'" class="flex-1 overflow-auto">
          <table class="w-full text-left text-[13px]">
            <thead class="bg-slate-50 border-b border-slate-200 sticky top-0 z-10 text-slate-600 font-semibold uppercase tracking-wider text-[11px]">
              <tr>
                <th class="px-5 py-3">Nombre Completo</th>
                <th class="px-5 py-3">Usuario (UPN)</th>
                <th class="px-5 py-3">Estado / ID</th>
                <th class="px-5 py-3 text-right">Acciones</th>
              </tr>
            </thead>
            <tbody class="divide-y divide-slate-100 text-slate-700">
              <tr v-if="loading" class="bg-white">
                <td colspan="4" class="px-5 py-12 text-center">
                  <span class="w-8 h-8 border-2 border-slate-200 border-t-indigo-600 rounded-full animate-spin inline-block mb-2"></span>
                  <p class="text-sm text-slate-500 font-medium">Buscando sesiones activas...</p>
                </td>
              </tr>
              <tr v-else-if="windowsSessions.length === 0" class="bg-white">
                <td colspan="4" class="px-5 py-12 text-center text-slate-500">
                  <svg class="w-12 h-12 text-slate-300 mx-auto mb-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                  <p class="text-[13px] font-medium">No hay sesiones de Windows activas.</p>
                </td>
              </tr>
              <tr v-for="session in windowsSessions" :key="session.username" class="hover:bg-slate-50 transition-colors">
                <td class="px-5 py-2">
                  <div class="font-medium text-slate-800">{{ session.full_name }}</div>
                  <div class="text-[10px] text-slate-400 mt-0.5" v-if="session.has_file">
                ...
                <td class="px-5 py-2">
                  <div class="flex items-center gap-2">
                    ...
                    <span class="text-[11px] font-mono text-slate-500 border border-slate-200 px-1.5 py-0.5 rounded bg-white">ID: {{ session.session_id }}</span>
                  </div>
                </td>
                <td class="px-5 py-2 text-right">
                  <button 
                  ...
                  >
                    <svg v-if="session.state === 'Borrando...'" class="w-3 h-3 border-2 border-red-600 border-t-transparent rounded-full animate-spin shrink-0"></svg>
                    <svg v-else class="w-3.5 h-3.5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" /></svg>
                    <span>Logoff (Cerrar)</span>
                  </button>
                </td>
              </tr>
            </tbody>
