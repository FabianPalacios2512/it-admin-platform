...
const searchOptions = [
  ...
  { name: 'Diagnóstico IT con IA', path: '/diagnostico', type: 'Módulo' },
  { name: 'Diagnosticar Equipo Remoto', path: '/diagnostico', type: 'Acción' }
]
...
// Lógica de inactividad dinámica según la ruta
...
const resetInactivityTimer = () => {
  ...
  if (limit === null) return // No programar logout automático en estas pantallas
...
onMounted(() => {
  ...
  navigationGroups.value.forEach(g => g.items.forEach(i => { if(i.subItems && isActive(i)) activeCategory.value = i.name }))
})
...
<template>
  <div class="flex h-screen overflow-hidden bg-white">
    ...
    <aside
      :class="[
        'hidden md:flex flex-col bg-slate-800 border-r border-slate-700/50 transition-all duration-200 ease-in-out shrink-0 z-20',
        sidebarWidth
      ]"
    >
      <!-- Brand -->
      <div class="flex items-center h-12 px-4 border-b border-slate-700/50">
        ...
        <template v-for="(group, gIdx) in navigationGroups" :key="gIdx">
          ...
          <ul class="space-y-0.5">
            <li v-for="item in group.items" :key="item.name" class="relative group">
              <template v-if="!item.subItems">
                ...
                >
                  <div class="flex items-center gap-3">
                    <svg
                      :class="['w-5 h-5 shrink-0 stroke-[1.5]', isActive(item) ? 'text-blue-400' : 'text-slate-500']"
                  ...
                  <span v-if="item.badge && !sidebarCollapsed" class="ml-auto text-[10px] px-1.5 py-0.5 rounded-md font-bold bg-fuchsia-500/10 border border-fuchsia-500/30 text-fuchsia-400">
                    {{ item.badge }}
                  </span>
                </router-link>
              </template>
              ...
              <template v-else>
                <div
                  @click="if(!sidebarCollapsed) { activeCategory = activeCategory === item.name ? null : item.name; }"
                  :class="[
                ...
                >
                  <div class="flex items-center gap-3">
                    <svg
                      :class="['w-5 h-5 shrink-0 stroke-[1.5]', isActive(item) ? 'text-blue-400' : 'text-slate-500 group-hover:text-slate-300']"
                      fill="none" viewBox="0 0 24 24" stroke="currentColor"
                    >
                      <path stroke-linecap="round" stroke-linejoin="round" :d="item.icon" />
                    </svg>
                  ...
                  <svg v-show="!sidebarCollapsed" :class="['w-4 h-4 transition-transform duration-200', activeCategory === item.name ? 'rotate-180' : '']" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <!-- Flyout Mode (Cerrado) -->
                <ul v-show="sidebarCollapsed" class="absolute left-full top-0 w-48 bg-slate-800 rounded-r-lg shadow-2xl invisible opacity-0 group-hover:visible group-hover:opacity-100 transition-all z-50 p-2 space-y-1">
                  <div class="px-2 pb-2 mb-1">
                    <span class="text-[11px] font-bold text-slate-400 uppercase tracking-wider">{{ item.name }}</span>
                  </div>
                  <li v-for="subItem in item.subItems" :key="'flyout-'+subItem.path">
                    <router-link
                      :to="subItem.path"
                      :class="[
                        'flex items-center py-2 px-2 transition-all duration-150 rounded text-[13px]',
                        isActive(subItem)
                    ...
                    >
                      <template v-if="subItem.icon">
                        <svg class="w-4 h-4 mr-2" :class="isActive(subItem) ? 'text-blue-500' : 'text-slate-500'" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" :d="subItem.icon" />
                        </svg>
                      </template>
                      {{ subItem.name }}
            ...
            <li v-for="item in group.items" :key="item.name">
              <template v-if="!item.subItems">
                ...
                >
                  ...
                  <span v-if="item.badge" class="text-[10px] px-1.5 py-0.5 rounded-md font-bold bg-fuchsia-500/10 border border-fuchsia-500/30 text-fuchsia-400">
                    {{ item.badge }}
                  </span>
                </router-link>
              </template>
              <template v-else>
                <div
                  @click="activeCategory = activeCategory === item.name ? null : item.name"
                  :class="[
                ...
                >
                  ...
                  <svg :class="['w-4 h-4 transition-transform duration-200', activeCategory === item.name ? 'rotate-180' : '']" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                  </svg>
                </div>
    ...
    <div class="flex flex-col flex-1 overflow-hidden">
      ...
      <header class="flex items-center justify-between h-12 px-4 sm:px-6 bg-white border-b border-gray-200 shadow-sm shrink-0 z-10 relative">
        ...
        <div class="flex items-center gap-3">
          ...
          <div class="hidden md:flex items-center gap-2 px-3 py-1.5 bg-slate-800 border border-slate-700">
            <span class="w-1.5 h-1.5 rounded-full bg-blue-500"></span>
            <span class="text-[10px] text-slate-400 font-mono uppercase tracking-wider">Connected to</span>
            <span class="text-[10px] text-white font-mono font-semibold">DC-01</span>
          </div>
      ...
      <main class="flex-1 min-h-0 p-4 sm:p-6 bg-slate-50 overflow-y-auto">
        <router-view />
      </main>
    </div>
  </div>
