    ...
    for _ in range(60):
        ...
        if os.path.exists(unc_file_path):
            try:
                ...
                # El archivo podría estar bloqueado mientras se escribe
                pass
...
def get_printers():
    """Obtiene la lista de impresoras y sus puertos TCP/IP."""
    global _printers_cache
    if time.time() - _printers_cache["time"] < _PRINTERS_CACHE_TTL and _printers_cache["data"] is not None:
        return _printers_cache["data"]

    script = """
    $printers = Get-Printer | Select-Object Name, PortName, Shared, ShareName, DriverName
    $ports = Get-PrinterPort | Select-Object Name, PrinterHostAddress

    $portMap = @{}
    foreach ($port in $ports) {
        if ($port.PrinterHostAddress) {
            $portMap[$port.Name] = $port.PrinterHostAddress
        } else {
            $portMap[$port.Name] = $port.Name
        }
    }

    $resultList = @()
    foreach ($p in $printers) {
        $ip = $portMap[$p.PortName]
    ...
    try:
        ...
        if isinstance(data, dict):
            ...
            res = data
        _printers_cache["data"] = res
        _printers_cache["time"] = time.time()
        return res
    except json.JSONDecodeError:
        print(f"Error decodificando JSON de impresoras: {output}")
        return []
