...
function Show-Banner {
    ...
    # -------------------------------------------------------------------------
    # BANNERS ROTATIVOS  Arte ASCII Cyber-Corp (3 diseos)
    # -------------------------------------------------------------------------
    ...
    Write-Host "MODO SEGURO: Polticas de solo lectura aplicadas  |  CANAL: Zero Trust Tunnel Cifrado`n" -ForegroundColor $clrSecondary
    ...
    Write-Host " Abortar sesin en cualquier momento.`n" -ForegroundColor $clrDim
}
...
function Invoke-SafeCommand {
    ...
    try {
        ...
        if ($null -eq $completed) {
            ...
            $result.stderr = "TIMEOUT: El comando excedi $Timeout segundos."
            $result.exit_code = -1
        }
        else {
...
# 3. nica lnea elegante de conexin
...
try {
    ...
    while ($sessionActive -and $ws.State -eq [System.Net.WebSockets.WebSocketState]::Open) {
        ...
        switch ($msgType) {
            ...
            "execute_command" {
                ...
                Write-Host "  | Hiptesis / Paso $stepNumber" -ForegroundColor Cyan
    ...
    if ($ex -is [System.Net.WebSockets.WebSocketException]) {
        Write-Host "`n  [DESCONEXION] La conexin con el servidor se perdi o fue cerrada abruptamente." -ForegroundColor Yellow
